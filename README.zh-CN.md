...
- 真实 LangChain LLM 接入，自动兼容 Anthropic/OpenAI 两种协议格式
- SSE 聊天流
...
- 主智能体：`LangChain 集成，兼容 Anthropic/OpenAI 协议的 LLM`
...
- LLM 和 项目知识库 都走真实 provider，不允许静默 fallback 成本地假实现
- 失败就报失败，未配置就报未配置
- 后端智能体运行时以 `backend/` 作为 project cwd 运行，因此只读取 `backend/CLAUDE.md` 和 `backend/.claude/skills/**`
...
- Python `3.11+`
- Node.js `18+`
- 可正常联网，供 LLM provider 和 项目知识库 使用
...
后端会自动读取 `backend/.env.local`。系统会根据 `LLM_BASE_URL` 和 `LLM_MODEL` 自动识别是 Anthropic 还是 OpenAI 格式。
...
cd backend
source .venv/bin/activate
python -c "from app.config import AppSettings; from app.services.llm_model import resolve_llm_config; api_key, base_url, model, fmt = resolve_llm_config(AppSettings()); print('LLM config ok: 模型=', model, '接口格式=', fmt); print('API 密钥已配置：', bool(api_key))"
```
...
1. 打开首页，确认项目列表能正常加载
2. 看首页或工作台的 provider readiness，确认 `LLM` 和 `项目知识库 RAG` 不是未配置状态
...
- LLM 构建：`backend/app/services/llm_model.py`
...
### 1. `ModuleNotFoundError: No module named 'langchain_openai'` / `No module named 'langchain_anthropic'`
...
### 2. `RAG provider 未配置或项目 knowledge base 未初始化`
...
### 4. 音频上传显示 `not_configured` 或 `normalization_failed`
