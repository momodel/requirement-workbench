...
def test_local_qdrant_client_disables_thread_check_for_fastapi_threadpool(tmp_path: Path, monkeypatch) -> None:
    settings = make_settings(tmp_path)
    store = QdrantLlamaIndexVectorStore(settings)
    captured: dict[str, object] = {}
    ...
    monkeypatch.setattr(store, "_load_qdrant_client_class", lambda: FakeClient)
