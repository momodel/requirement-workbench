...
def test_coerce_html_artifact_payload_parses_marker_format_with_blank_lines() -> None:
    raw = """
TITLE: 集团业财逐笔对账系统方案

SUMMARY: 围绕订单/结算系统与财务系统的金额一致性校验，建立差异识别与人工确认闭环。

HTML:
<!doctype html>
<html>
  <head><title>集团业财逐笔对账系统方案</title></head>
  <body><main>ok</main></body>
</html>
    """.strip()
    ...
    assert parsed["title"] == "集团业财逐笔对账系统方案"
    assert parsed["summary"] == "围绕订单/结算系统与财务系统的金额一致性校验，建立差异识别与人工确认闭环。"
    assert "<main>ok</main>" in parsed["html"]
...
def test_structured_prompt_requires_non_empty_state_items_for_explicit_state_requests(tmp_path: Path) -> None:
    methodology_dir = tmp_path / "backend" / ".claude" / "skills" / "requirement-analysis-methodology"
    evidence_dir = tmp_path / "backend" / ".claude" / "skills" / "notebooklm-evidence-workflow"
    methodology_dir.mkdir(parents=True, exist_ok=True)
    evidence_dir.mkdir(parents=True, exist_ok=True)
    ...
    prompt = runtime._build_structured_prompt(
        AgentTurnInput(
            ...
            user_message="请把这条规则写入当前理解，并列出至少一个待确认项。",
            selected_source_ids=[],
            source_summaries=["规则说明"],
            evidence_summary="命中了规则文本。",
            evidence_citations=[],
            request_artifact_types=[],
        ),
        assistant_message="我会先确认这条规则的影响。",
    )

    assert "至少一个状态桶必须产出新增条目" in prompt
    assert "current_understanding 至少输出 1 条" in prompt
    assert "pending_items 至少输出 1 条" in prompt
